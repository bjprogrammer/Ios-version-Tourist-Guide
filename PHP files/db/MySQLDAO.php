...
class MySQLDAO
{
    ...
    public function getfbUserDetails($email)
    {
        $returnValue = array();
        $sql = "select * from fbuser where email='" . $email . "'";
...
public function gettwUserDetails($email)
    {
        $returnValue = array();
        $sql = "select * from twuser where email='" . $email . "'";
...
public function fbUser($email, $first_name, $last_name, $fbid)
    { 
        $sql = "insert into fbuser set email=?, first_name=?, last_name=?, fbid=?";
        $statement = $this->conn->prepare($sql);

        if (!$statement)
            throw new Exception($statement->error);

        $statement->bind_param("ssss", $email, $first_name, $last_name, $fbid);
        $returnValue = $statement->execute();

        return $returnValue;  
    }   

public function twUser($email, $first_name, $last_name,$twid)
    { 
        $sql = "insert into twuser set email=?, first_name=?, last_name=?, twid=?";
        $statement = $this->conn->prepare($sql);

        if (!$statement)
            throw new Exception($statement->error);

        $statement->bind_param("ssss", $email, $first_name, $last_name, $twid);
        $returnValue = $statement->execute();
